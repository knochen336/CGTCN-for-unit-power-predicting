...
from net.GTCN_new_ontcn_24_cons import *
import pandas as pd
import numpy as np
...
def train():
    ...
    for epoch in trange(EPOCH, desc="Epochs"):
        ...
        print("validmaeloss:", epoch_val_loss, "valsmoothf1", smoothf1_val, "testmaeloss:", epoch_test_loss)
        if epoch_val_loss < best_val_loss:
            best_val_loss = epoch_val_loss
            torch.save(model.state_dict(),
                       f'./model_save_118_new/model_best{BATCHSIZE}_{lEARINING_RATE}_{EPOCH}_24_cons.pth')
        scheduler1.step()
        scheduler2.step()
    torch.save(model.state_dict(),
               f'./model_save_118_new/model_last{BATCHSIZE}_{lEARINING_RATE}_{EPOCH}_24_cons.pth')
...
model = Net().to(device)
pathstr = f'./model_save_118_new/model_best32_0.0008_400_length24.pth'  # 0.0999
state = torch.load(pathstr, map_location=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
model.load_state_dict(state)
# pathstr = f'./model_best16_0.001_300modeltrain.pth'  # 0.0999
# state = torch.load(pathstr, map_location=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
# model.load_state_dict(state)

optimizer = torch.optim.Adam(model.parameters(), lr=lEARINING_RATE, weight_decay=1e-5)  # 5e-4
# scheduler1 = MultiStepLR(optimizer, milestones=[170, 190, 210, 230, 280, 330, 370], gamma=0.5)  # 原来没有265
# scheduler2 = MultiStepLR(optimizer,
#                          milestones=[50, 70, 90, 110, 130, 150],
#                          gamma=0.8)
scheduler1 = MultiStepLR(optimizer, milestones=[170, 190, 210, 230, 280, 330, 370], gamma=0.5)  # 原来没有265
scheduler2 = MultiStepLR(optimizer,
                         milestones=[50, 70, 90, 110, 130, 150],
...
plt.legend()
plt.show()
with open('cgtcn_new_24_losses_and_mse_smoothl1.csv', 'w', newline='') as file:
