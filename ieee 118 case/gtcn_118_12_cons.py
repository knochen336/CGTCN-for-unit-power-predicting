    ...
    for epoch in trange(EPOCH, desc="Epochs"):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       f'./model_save_118_new/model_best{BATCHSIZE}_{lEARINING_RATE}_{EPOCH}_12_cons.pth')
        scheduler1.step()
        scheduler2.step()
    torch.save(model.state_dict(),
               f'./model_save_118_new/model_last{BATCHSIZE}_{lEARINING_RATE}_{EPOCH}_12_cons.pth')
...
model = Net().to(device)
pathstr = f'./model_save_118_new/model_best32_0.0008_400.pth'  # 0.0999
...
plt.legend()
plt.show()
with open('cgtcn_new_12_losses_and_mse_smoothl1.csv', 'w', newline='') as file:
