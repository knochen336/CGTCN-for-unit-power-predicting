...
max_change = 20
# 将上限和下限复制12次
p_max_12 = np.tile(p_max, 24)
p_min_12 = np.tile(p_min, 24)
x_length = 24
y_length = 24
import numpy as np
...
constraints = [
    # 所有元素都大于等于0
    LinearConstraints(
        A=-np.eye(54 * 24),  # 注意这里是负的单位矩阵，因为我们要表示的是 x >= 0，而不是 x <= 0
...
class Net(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x = F.relu(x)
        # x = self.conv3(x, edge_index, edge_weight) + x2 #[batch_size*num_nodes, num_features]
        # x = F.relu(x)
        # x3 = x
        x = x.view(-1, 118,
